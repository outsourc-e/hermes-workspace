...
export function syncSwarmProfileModel(
  ...
  const existingBaseUrl =
    existingModel && typeof existingModel.base_url === 'string'
      ? existingModel.base_url
      : ''
  // A base_url belongs to a specific provider. When it's non-empty but the
  // provider is being (re)asserted to ollama-cloud, the base_url is stale (the
  // recurring `provider: deepseek` + api.deepseek.com drift) and must be
  // cleared, or hermes still targets the dead endpoint. ollama-cloud uses its
  // built-in endpoint, so an empty base_url is correct.
  const baseUrlNeedsClear =
    next.provider === 'ollama-cloud' && existingBaseUrl !== ''
  ...
  // Clear a stale provider-specific endpoint when switching provider.
  if (baseUrlNeedsClear || existingProvider !== next.provider) {
    merged.base_url = ''
  }
  root.model = merged
