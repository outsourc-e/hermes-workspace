...
describe('deriveTomorrow', () => {
  ...
  function ev(start: string, summary: string, feed_name = 'Personal', is_all_day = false) {
  ...
  it('counts multiple events and shows first + remainder', () => {
    ...
    const t9 = new Date(tStart); t9.setHours(9, 30, 0, 0);
    const t13 = new Date(tStart); t13.setHours(13, 0, 0, 0);
    const t17 = new Date(tStart); t17.setHours(17, 0, 0, 0);
    const data = deriveTomorrow(
      [
  ...
  it('marks all-day events as ALL DAY', () => {
    ...
    const data = deriveTomorrow(
      [{ ...ev(tomorrowMid.toISOString(), 'Public holiday', 'Family'), is_all_day: true }],
      now,
      TEST_TZ,
    );
