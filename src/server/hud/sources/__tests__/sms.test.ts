...
describe('smsAdapter', () => {
  it('returns connected when platform state is connected', async () => {
    fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ platforms: { sms: { state: 'connected' } } }) });
  ...
  it('returns disconnected when platform state is not connected', async () => {
    fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ platforms: { sms: { state: 'disconnected' } } }) });
    const r = await smsAdapter.fetch();
