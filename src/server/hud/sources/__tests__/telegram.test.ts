...
describe('telegramAdapter', () => {
  it('returns connected when platform state is connected', async () => {
    fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ platforms: { telegram: { state: 'connected' } } }) });
  ...
  it('returns disconnected when platform state is not connected', async () => {
    fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ platforms: { telegram: { state: 'disconnected' } } }) });
    const r = await telegramAdapter.fetch();
  ...
  it('returns unknown when platform missing', async () => {
    fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ platforms: {} }) });
    const r = await telegramAdapter.fetch();
    expect(r.value).toBe('?');
