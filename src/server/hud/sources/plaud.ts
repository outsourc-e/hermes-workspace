...
interface PlaudData { value: string; sub: string; tone: 'ok' | 'info'; }
...
export function computePlaudStat(input: RawPlaud[] | PlaudResponse): PlaudData {
  const recordings: RawPlaud[] = Array.isArray(input) ? input : (input?.data ?? []);
