...
const HEALTH_URL = process.env.HERMES_HEALTH_URL || 'http://127.0.0.1:8642/health/detailed';

interface TelegramData { value: string; sub: string; tone: 'ok' | 'warn' | 'err'; }
interface HealthResponse { platforms?: Record<string, { state?: string }>; }
...
export const telegramAdapter: SourceAdapter<TelegramData> = {
  ...
  async fetch() {
    ...
    if (state === 'connected') return { value: 'up', sub: 'telegram bot', tone: 'ok' };
