...
export function computeWhoopData(w: RawWhoop): WhoopData {
  ...
  if (w.resting_hr_bpm !== undefined) statsParts.push(`RHR ${Math.round(w.resting_hr_bpm)}`);
  if (w.sleep_hours !== undefined) statsParts.push(`Sleep ${w.sleep_hours.toFixed(1)}h`);
  if (w.day_strain !== undefined) statsParts.push(`Strain ${w.day_strain.toFixed(1)}`);

  // Recovery band as label tag.
  const band = recovery >= 67 ? 'GREEN' : recovery >= 34 ? 'YELLOW' : recovery > 0 ? 'RED' : '—';
