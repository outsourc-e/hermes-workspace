...
function categorise(name?: string): 'work' | 'uni' | 'clinic' | 'personal' | 'family' {
...
export function deriveTomorrow(
  events: Array<{ start: string; end: string; summary: string; feed_name: string; is_all_day: boolean }>,
  now: Date = new Date(),
  ...
  const tags = new Set(tomorrowEvents.map((e) => categorise(e.feed_name).toUpperCase()));
