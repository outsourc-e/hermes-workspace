...
describe('buildInboxFeed', () => {
  it('orders urgents first, then warnings, then info', () => {
    const items = [
      { id: 'a', severity: 'info' as const, tag: 'AGENT', body: 'qa done', when: '2m' },
      { id: 'b', severity: 'urgent' as const, tag: 'URGENT', body: 'meeting now', when: '14:30' },
      { id: 'c', severity: 'warn' as const, tag: 'UNI', body: 'due fri', when: 'Fri' },
    ];
  ...
  it('promotes starred SMS contacts to urgent', () => {
    const cfg = { ...defaultHUDConfig, inbox_severity_overrides: { starred_sms_contacts: ['+61400000000'] } };
    const items = [
      { id: 'sms-1', severity: 'info' as const, tag: 'SMS', body: 'from +61400000000: yo', when: 'now' },
    ];
  ...
  it('filters dismissed items still within ttl', () => {
    const cfg = { ...defaultHUDConfig, dismissed_inbox_items: { 'a': Date.now() + 60_000 } };
    const items = [
