export type WidgetState = 'loaded' | 'loading' | 'stale' | 'errored' | 'disabled';
...
export interface HUDConfig {
  ...
  mc_tile_order?: WidgetId[];              // user-reordered MC tiles
  inbox_severity_overrides?: {
    starred_sms_contacts?: string[];       // phone numbers → urgent
  };
  dismissed_inbox_items?: Record<string, number>;  // item-key → dismissed-until (unix ms)
  mobile_tiles?: WidgetId[];               // subset shown on mobile
}
