...
function sanitizeModelOverride(
  ...
  if (!normalized || normalized === 'auto' || normalized === 'auto/coding')
    return null
...
function dashboardTaskToCard(task: DashboardKanbanTask): SwarmKanbanCard {
  ...
  return {
    ...
    modelOverride: task.model_override ?? null,
...
const claudeBackend: KanbanBackend = {
  ...
  create(input) {
    ...
    const statements = [
      'begin immediate;',
      'insert into tasks (',
      'id, title, body, assignee, status, priority, created_by, created_at, workspace_kind, workspace_path, idempotency_key, model_override, reasoning_effort',
      ') values (',
      [
        sqliteQuote(taskId),
        ...
        String(nowSeconds),
        sqliteQuote(
          input.workspaceKind ?? (input.workspacePath ? 'worktree' : 'scratch'),
        ),
        sqliteQuote(
        ...
        input.modelOverride?.trim()
          ? sqliteQuote(input.modelOverride.trim())
          : 'NULL',
        input.reasoningEffort?.trim()
          ? sqliteQuote(input.reasoningEffort.trim())
          : 'NULL',
      ].join(', '),
      ');',
      ...linkStatements,
...
const dashboardProxyBackend: KanbanBackend = {
  ...
  async create(input) {
    const task = await createDashboardKanbanTask({
      ...
      workspace_kind:
        input.workspaceKind ?? (input.workspacePath ? 'worktree' : undefined),
      workspace_path: input.workspacePath?.trim() || undefined,
      model_override: input.modelOverride?.trim() || undefined,
      reasoning_effort: input.reasoningEffort?.trim() || undefined,
    })
...
export async function createKanbanCard(
  ...
  const sanitized: CreateSwarmKanbanCardInput = {
    ...input,
    modelOverride: sanitizeModelOverride(input.modelOverride),
  }
  return Promise.resolve(resolveKanbanBackend().create(sanitized))
}
...
export async function updateKanbanCard(
  ...
  const sanitized: UpdateSwarmKanbanCardInput = {
    ...updates,
    modelOverride: sanitizeModelOverride(updates.modelOverride ?? null),
  }
  return Promise.resolve(resolveKanbanBackend().update(cardId, sanitized))
}
