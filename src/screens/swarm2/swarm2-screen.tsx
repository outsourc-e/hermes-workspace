...
import { GlobalPermissionModeControl } from './swarm2-permission-mode'
...
import { Swarm2QueuePanel } from './swarm2-queue-panel'
...
async function setSwarmMode(mode: 'auto' | 'manual'): Promise<SwarmModeData> {
  const res = await fetch('/api/swarm-mode', {
    method: 'POST',
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        />
        <GlobalPermissionModeControl className="w-full max-w-5xl" />
...
export function Swarm2Screen() {
  ...
  const swarmModeQuery = useQuery({
    queryKey: ['swarm2', 'mode'],
    queryFn: fetchSwarmMode,
    refetchInterval: 30_000,
  })
  const swarmModeMutation = useMutation({
    ...
    onError: (_err, _newMode, context) => {
      if (context?.previous) {
        queryClient.setQueryData(['swarm2', 'mode'], context.previous)
      }
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['swarm2', 'mode'] })
    },
  })
  const swarmMode = swarmModeQuery.data?.mode ?? 'auto'
  const isAutoMode = swarmMode === 'auto'
        ...
        <header className="rounded-xl border border-primary-200 bg-primary-50/80 px-5 py-3 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex min-w-0 flex-wrap items-center gap-3">
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-[var(--theme-border)] bg-[var(--theme-card)] px-2.5 py-1.5 shadow-sm">
                <span className="text-[10px] font-medium uppercase tracking-[0.15em] text-[var(--theme-muted)]">
                  Approvals
                </span>
                <button
                  type="button"
                  ...
                  className={cn(
                    ...
                    isAutoMode
                      ...
                      : 'border-[var(--theme-border2)] bg-[var(--theme-card2)]',
                  )}
                >
                  <span
                    className={cn(
