          ...
          <div className="absolute right-0 top-0 flex shrink-0 items-center gap-1">
            ...
            {onClearAll ? (
              <button
                type="button"
                onClick={onClearAll}
                className="inline-flex h-9 items-center gap-1 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card)] px-2.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--theme-danger,#e5484d)] hover:bg-[var(--theme-card2)]"
                aria-label="Clear all tasks and stop all workers"
                title="Stop all workers, cancel all tasks, clear the blocked board"
              >
                <HugeiconsIcon
                  icon={Cancel01Icon}
                  size={13}
