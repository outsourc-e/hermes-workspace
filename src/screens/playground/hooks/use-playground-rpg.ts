...
export function usePlaygroundRpg() {
  ...
  const markObjective = useCallback(
    (questId: string, objectiveId: string) => {
      ...
      if (!quest) return
      const completedQuest: { current: PlaygroundQuest | null } = {
        current: null,
      }
      setState((prev) => {
        ...
        completedQuest.current = quest
        return completeQuestState(next, quest)
      })
      if (completedQuest.current) {
        const q = completedQuest.current
        pushToast('quest', 'Quest Complete', q.title)
        pushToast('xp', '+ XP', `+${q.reward.xp} XP`)
        if (q.reward.items?.length) {
        ...
        if (q.reward.title) {
          pushToast('title', 'Title Unlocked', q.reward.title)
        }
      }
    },
    [pushToast],
