...
function completeQuestState(prev: PlaygroundRpgState, quest: PlaygroundQuest) {
  ...
  const unlockedWorlds = Array.from(
    new Set([
      ...prev.unlockedWorlds,
      ...(reward.unlockWorlds === undefined ? [] : reward.unlockWorlds),
    ]),
  )
...
export function usePlaygroundRpg() {
  ...
  const stats = useMemo(() => {
    ...
    for (const itemId of Object.values(state.playerProfile.equipped)) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- runtime safety: optional chain protects against nullable field not visible to TS
  ...
  const markObjective = useCallback(
    (questId: string, objectiveId: string) => {
      ...
      setState((prev): PlaygroundRpgState => {
      ...
      const reward = completedQuest?.reward
      if (completedQuest) {
        pushToast('quest', 'Quest Complete', completedQuest.title)
        if (reward) {
          pushToast('xp', '+ XP', `+${reward.xp} XP`)
          ...
          if (reward.title) {
            pushToast('title', 'Title Unlocked', reward.title)
          }
        }
      }
    },
