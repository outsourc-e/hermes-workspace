...
const DEFAULT_UNLOCKED_WORLDS: Array<PlaygroundWorldId> = ['training', 'agora']
...
function currentObjectiveForQuest(
  ...
  const progress = (
    state.playerProfile.questProgress as Partial<
...
function completeQuestState(prev: PlaygroundRpgState, quest: PlaygroundQuest) {
  ...
  const inventory = Array.from(
    new Set([
      ...valueOr(prev.playerProfile.inventory, []),
      ...(reward.items ?? []),
    ]),
  )
  ...
  const unlockedWorlds = Array.from(
    new Set([
      ...valueOr(prev.unlockedWorlds, DEFAULT_UNLOCKED_WORLDS),
      ...(reward.unlockWorlds ?? []),
    ]),
  )
  ...
  for (const [skill, amount] of Object.entries(reward.skillXp ?? {})) {
    skillXp[skill as PlaygroundSkillId] =
      valueOr(skillXp[skill as PlaygroundSkillId], 0) + valueOr(amount, 0)
  }
...
export function usePlaygroundRpg() {
  ...
  const markObjective = useCallback(
    (questId: string, objectiveId: string) => {
      ...
      if (!quest) return
      const completion: { quest: PlaygroundQuest | null } = { quest: null }
      setState((prev) => {
      ...
      const completedQuest = completion.quest
      if (completedQuest) {
      ...
      setState((prev) => {
        ...
        for (const [skill, amount] of Object.entries(skillXp)) {
          next[skill as PlaygroundSkillId] =
            valueOr(next[skill as PlaygroundSkillId], 0) + valueOr(amount, 0)
        }
