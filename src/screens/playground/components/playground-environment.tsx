...
export function Lantern({
  ...
  const ref = useRef<MaterialMesh>(null)
  useFrame(({ clock }) => {
    if (!ref.current) return
    const t = clock.getElapsedTime()
    ref.current.material.emissiveIntensity = 1.6 + Math.sin(t * 5) * 0.3
  })
  return (
    <group position={position}>
      <mesh castShadow position={[0, 0.6, 0]}>
...
export function FlowerPatch({
  ...
  const items = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      out.push({
        pos: [dx, 0, dz],
        color: palette[Math.floor(r() * palette.length)] ?? '#fde68a',
      })
    }
    return out
...
export function Fountain({
  ...
  const splashRef = useRef<TransformNode>(null)
...
export function EnergyCore({
  ...
  const ref = useRef<TransformNode>(null)
  useFrame(({ clock }) => {
...
function InstancedRocks({ items }: { items: Array<SceneryInstance> }) {
  const ref = useRef<InstancedMeshNode>(null)
  const scene = useThree((state) => state.scene)
  const matrices = useMemo(() => {
    const dummy = scene.clone(false)
