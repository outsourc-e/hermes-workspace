...
import type { DialogChoice, NpcDialogTree } from '../lib/npc-dialog'
import type {
  PlaygroundItemId,
  PlaygroundQuest,
  PlaygroundSkillId,
...
export function PlaygroundDialog({
  ...
  const npc = (NPC_DIALOG as Partial<Record<string, NpcDialogTree>>)[
    activeNpcId
  ]
  if (!npc) return null
  const activeNpc = npc
  ...
  function handleNextLore() {
    const nextLore = activeNpc.lore[loreIdx % activeNpc.lore.length]
    if (!nextLore) return
    setShowLore(true)
    setReply(nextLore)
    setLoreIdx((i) => i + 1)
