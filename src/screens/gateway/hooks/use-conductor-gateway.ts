  ...
  const sendMission = useMutation({
    mutationFn: async ({
      ...
      if (result.mode === 'native-swarm') {
        const resultMissionId = result.missionId ?? null
        setMissionId(resultMissionId)
        setMissionJobId(result.jobId ?? null)
        setOrchestratorSessionKey(resultMissionId)
        if (resultMissionId) {
          setMissionWorkerKeys((current) => {
            if (current.has(resultMissionId)) return current
            const next = new Set(current)
            next.add(resultMissionId)
            return next
          })
        }
        setPlanText(
