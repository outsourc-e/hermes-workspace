...
/**
 * Fleet hibernate toggle: stop every background job (sweeps, watchdog,
 * schedules, discord bot) when the operator is away; wake restores all.
 * Only the workspace UI stays up so this button keeps working.
 */
function HibernateButton() {
  const [state, setState] = useState<'awake' | 'hibernating' | 'busy'>('awake')
  useEffect(() => {
    fetch('/api/hibernate')
      .then((r) => r.json())
      .then((d) => {
        if (d?.state === 'hibernating') setState('hibernating')
      })
      .catch(() => {})
  }, [])
  const toggle = async () => {
    const next = state === 'hibernating' ? 'start' : 'stop'
    if (
      next === 'stop' &&
      !window.confirm(
        'Stop everything? All background agents, sweeps, schedules and the Discord bot go to sleep until you press Wake.',
      )
    )
      return
    setState('busy')
    try {
      ...
      setState(d?.state === 'hibernating' ? 'hibernating' : 'awake')
    } catch {
      setState('awake')
    }
  }
  return (
    <button
      ...
      onClick={toggle}
      disabled={state === 'busy'}
      className="fixed bottom-4 right-4 z-40 rounded-full border px-4 py-2 text-xs font-semibold shadow-lg transition-colors disabled:opacity-50"
      style={{
        background:
          state === 'hibernating' ? 'var(--theme-accent-soft)' : 'var(--theme-card)',
        borderColor: 'var(--theme-border)',
        color: 'var(--theme-text)',
      }}
      title={
        state === 'hibernating'
          ? 'Background fleet is asleep — click to wake everything'
          : 'Stop all background agents and schedules'
      }
    >
      {state === 'busy'
        ? '…'
