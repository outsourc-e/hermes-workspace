...
function terraAgentCurrentFocus(
  ...
  if (printer.state === 'ready' && (printer.cameraUrl || printer.snapshotUrl || printer.statusUrl)) {
    return {
      stationId: 'terra-printer-control',
      label: 'Checking live printer source',
      reason: 'A read-only printer/camera source is configured, so Terra keeps one hand on Printer Control.',
    }
  }
...
function generatedPlaceholderPreview(name: string, note = 'preview unavailable'): TerraModelAssetPreview {
  const label = safeSvgText(name.replace(/\.3mf$/i, '').slice(0, 28))
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="280" height="180" viewBox="0 0 280 180"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#e7f1df"/><stop offset="1" stop-color="#f8f3e7"/></linearGradient></defs><rect width="280" height="180" rx="18" fill="url(#g)"/><path d="M70 126 140 38 212 126Z" fill="#6f9f5f" opacity=".25" stroke="#416c32" stroke-width="5"/><text x="140" y="88" text-anchor="middle" font-family="system-ui, sans-serif" font-size="26" font-weight="900" fill="#2f5727">3MF</text><text x="140" y="142" text-anchor="middle" font-family="system-ui, sans-serif" font-size="13" font-weight="800" fill="#5a674f">${label}</text></svg>`
  return {
    kind: 'generated',
...
function generatedGeometryPreview(xml: string, name: string): TerraModelAssetPreview {
  ...
  const sampled = triangles
    ...
    .slice(-950)
    .map((triangle, index) => `<polygon points="${triangle.points}" fill="${index % 3 === 0 ? '#6f9f5f' : index % 3 === 1 ? '#8eb979' : '#426f36'}" fill-opacity=".42" stroke="#264d22" stroke-opacity=".24" stroke-width=".45"/>`)
    .join('')
  const body = sampled || vertices.slice(0, 900).map((vertex) => {
    ...
    return `<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="1.1" fill="#416c32" opacity=".5"/>`
  }).join('')
  const label = safeSvgText(name.replace(/\.3mf$/i, '').slice(0, 24))
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="280" height="180" viewBox="0 0 280 180"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#f7f4e8"/><stop offset="1" stop-color="#e8f3e4"/></linearGradient><filter id="shadow"><feDropShadow dx="0" dy="8" stdDeviation="8" flood-color="#1b2b18" flood-opacity=".18"/></filter></defs><rect width="280" height="180" rx="18" fill="url(#bg)"/><g filter="url(#shadow)">${body}</g><text x="16" y="24" font-family="system-ui, sans-serif" font-size="12" font-weight="900" fill="#2f5727">3MF PREVIEW</text><text x="16" y="162" font-family="system-ui, sans-serif" font-size="12" font-weight="800" fill="#5a674f">${label}</text></svg>`
  return {
    kind: 'generated',
    dataUrl: `data:image/svg+xml;base64,${Buffer.from(svg).toString('base64')}`,
  ...
  const discoveredPrinters: Array<TerraDiscoveredPrinter> = Object.entries(printerList)
    ...
    .map(([printerId, value]) => {
      ...
      return {
        ...
        serialNumber: stringValue(record.serialNumber) ?? stringValue(record.mainboardId),
        cameraUrl: host ? `http://${host}:8080/` : undefined,
...
export async function getTerraPrinterReadOnlyStatus(nowMs = Date.now()): Promise<TerraPrinterReadOnlyStatus> {
  ...
  const configured = Boolean(config.cameraUrl || config.snapshotUrl || config.statusUrl)
  ...
  const cameraProbe = config.cameraUrl ? await probeCamera(config.cameraUrl) : undefined
  ...
  const metrics = {
    ...statusMetrics,
    queueState: statusMetrics.queueState ?? (cameraProbe?.ready ? 'camera online' : undefined),
  ...
  return {
    ...
    state,
    message: state === 'ready'
      ? 'Live read-only printer camera/status connected.'
      : state === 'unreachable'
        ? 'Printer discovered, but the read-only camera/status endpoint is unreachable right now.'
        : 'Printer read-only config is present. Waiting for status/camera response.',
    lastCheckedAtMs: nowMs,
...
async function discoverElegooSlicerProfiles() {
  ...
  const selectedMachineProfile = machines.profiles.find((profile) => profile.default) ?? machines.profiles[0]
...
export async function getTerraWorkbenchCapabilities(nowMs = Date.now()): Promise<TerraWorkbenchCapabilities> {
  ...
  const workflow: Array<TerraWorkflowStep> = [
    ...
    { id: 'send-to-printer', label: 'Upload / start print', state: printerLive ? 'locked' : 'blocked', live: printerLive, locked: true, requiresApproval: true, source: printer.source, note: printerLive ? 'Printer is live, but upload/start remains approval-gated.' : printer.message },
    ...
    { id: 'record-print', label: 'Capture / timelapse', state: cameraLive ? 'ready' : cameraConfigured ? 'blocked' : 'blocked', live: cameraLive, locked: false, requiresApproval: false, source: '/api/war-room/terra-printer-frame', note: cameraLive ? 'Live JPEG frame proxy available.' : cameraConfigured ? 'Camera URL is configured, but frame probe is unreachable right now.' : 'No camera URL discovered.' },
    { id: 'post-print-qa', label: 'Post-print camera QA', state: cameraLive ? 'ready' : 'blocked', live: cameraLive, locked: false, requiresApproval: false, source: '/api/war-room/terra-print-qa', note: cameraLive ? 'After completion, Terra captures a real frame and flags blank/blur/stringy failure risk.' : 'QA needs a reachable real camera frame.' },
  ...
  const capabilities: Array<TerraWorkbenchCapability> = [
    ...
    { id: 'post-print-camera-qa', label: 'Post-print camera QA', category: 'camera', state: cameraLive ? 'ready' : 'blocked', live: cameraLive, locked: false, source: '/api/war-room/terra-print-qa', evidence: [cameraLive ? 'camera frame reachable' : cameraConfigured ? 'camera configured but unreachable' : 'camera URL missing', 'blank/dark/blur/stringing heuristics', 'shape match requires visual confirmation'], note: cameraLive ? 'Captures a real camera frame after completion/manual run and flags failure-risk signals.' : 'Blocked until a real camera frame is reachable; no fake QA pass.' },
    { id: 'machine-actions', label: 'Pause / resume / cancel / heat / upload', category: 'safety', state: printerLive ? 'locked' : 'blocked', live: printerLive, locked: true, source: 'approval gate', evidence: printer.lockedActions, note: 'Visible as real workflow actions, but no side effect runs without DLV approval and readback.' },
...
export async function getTerraPrinterCameraFrame(): Promise<TerraPrinterCameraFrameResult> {
  ...
  const sourceUrl = status.cameraUrl ?? status.snapshotUrl
  if (!sourceUrl) return { ok: false, status: 404, error: 'No read-only printer camera URL discovered yet.' }
  ...
  try {
    ...
    if (!response.ok || !response.body) {
      return { ok: false, status: response.status || 502, error: `Camera frame fetch failed: HTTP ${response.status}`, sourceUrl }
    }
    const contentType = response.headers.get('content-type') ?? ''
    ...
    return { ok: false, status: 502, error: 'No JPEG frame found in printer camera stream.', sourceUrl }
  } catch (error) {
    return { ok: false, status: 502, error: (error as Error).message, sourceUrl }
  } finally {
    controller.abort()
    clearTimeout(timeout)
  }
