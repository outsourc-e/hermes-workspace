...
describe('parseHUDConfig', () => {
  ...
  it('returns independent deep clones — mutating one result does not affect another', () => {
    ...
    expect((cfg2.dismissed_inbox_items as Record<string, unknown>)['poison']).toBeUndefined();
    ...
    expect((defaultHUDConfig.dismissed_inbox_items as Record<string, unknown>)['poison']).toBeUndefined();
    expect(defaultHUDConfig.mc_tile_order).not.toContain('injected');
    expect(defaultHUDConfig.inbox_severity_overrides.starred_sms_contacts).toHaveLength(0);
  });
});
