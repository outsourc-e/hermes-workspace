...
function callGet(url: string): Promise<Response> {
  const request = makeRequest(url)
  // TanStack's generated route type intentionally hides concrete server
  // handlers. The route module supplies this GET handler at runtime.
  const server = (
    Route.options as { server?: { handlers?: { GET?: GetHandler } } }
  ).server
