...
export function summarizeSwarmHealth(
  ...
  // RECURS: profile provider drift. hermes-agent rewrites a worker's provider
  // from the bare model name to deepseek/openrouter/custom (no key) → silent
  // fallback. Dispatch self-heals it, but surface it so it's visible early.
  const DRIFT_PROVIDERS = new Set(['deepseek', 'openrouter', 'custom'])
  const drifted = workers.filter((w) =>
    DRIFT_PROVIDERS.has((w.provider || '').toLowerCase()),
  )
  if (drifted.length > 0)
    warnings.push(
      `${drifted.length} worker(s) on a drifted provider (${drifted
        .map((w) => w.workerId)
        .join(', ')}); a dispatch re-asserts ollama-cloud.`,
    )
  const missingWrapper = workers.filter((w) => !w.wrapperFound)
  if (missingWrapper.length > 0)
    warnings.push(
      `${missingWrapper.length} worker(s) missing a launcher wrapper — run scripts/swarm-generate-wrappers.sh.`,
    )
  const missingProfile = workers.filter((w) => !w.profileFound)
  if (missingProfile.length > 0)
    warnings.push(
      `${missingProfile.length} worker(s) missing a profile directory.`,
    )
  return {
