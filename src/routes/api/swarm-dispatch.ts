...
function runWorker(
  ...
  // eslint-disable-next-line no-async-promise-executor -- needed for async setup; errors surface via the error handler callback
  return new Promise(async (resolve) => {
    ...
    const proc = execFile(
      ...
      (
        ...
        if (error) {
          const code =
            (error as unknown as { code?: number | null }).code ?? null
          const result: WorkerResult = {
            workerId,
            ok: false,
            output: out,
