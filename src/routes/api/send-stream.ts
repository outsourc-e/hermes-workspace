                  ...
                  if (useResponsesApi) {
                    ...
                    try {
                      ...
                      for await (const ev of responsesStream) {
                        ...
                        // At this point ev.kind is 'failed' (the only remaining
                        // variant after the earlier continue/break arms).
                  ...
                  },
                  {
                    ...
                    async onEvent({ event, data }) {
                      ...
                      if (event === 'assistant.delta') {
                        ...
                        sendEvent('chunk', translated)
                        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- runtime safety: defensive check; lint can't see store mutation
                        if (!skipPublish) publishChatEvent('chunk', translated)
                        return
                      }
                      ...
                      if (
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- runtime safety: defensive check; lint can't see store mutation
                        if (!skipPublish) publishChatEvent('tool', translated)
                        return
                      }
                      ...
                      if (event === 'run.completed') {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- runtime safety: defensive check; lint can't see store mutation
                        if (!skipPublish) publishChatEvent('done', translated)
                        closeStream()
                      }
                    },
                  },
