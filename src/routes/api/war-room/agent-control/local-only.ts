...
import { getAgentConnectionStoreInfo, setWarRoomAgentsLocalOnly } from '../../../../lib/war-room/body'
...
export const Route = createFileRoute('/api/war-room/agent-control/local-only')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const state = setWarRoomAgentsLocalOnly({
