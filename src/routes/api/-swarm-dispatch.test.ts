...
describe('dispatchBlockReason', () => {
  it('only blocks genuine failures, not successful-but-uncheckpointed runs', () => {
    expect(
      dispatchBlockReason({
        ok: false,
    ...
    // A successful dispatch that merely lacks a fresh checkpoint is NOT a
    // block — the worker ran the task; it just didn't emit the structured
    // checkpoint block. This previously produced false "session timed out"
    // blocks on completed work.
    expect(
      dispatchBlockReason({
        ok: true,
        error: null,
