      ...
      GET: async ({ request }) => {
        ...
        try {
          ...
          if (getGatewayCapabilities().models) {
            // Non-fatal: a transient upstream auth/availability failure here
            // must not discard the models already gathered from models.json,
            // config.yaml, and local discovery. Merge on success, skip on error.
            try {
              ...
              // ignore upstream hermes-agent /v1/models failure
            }
          }
