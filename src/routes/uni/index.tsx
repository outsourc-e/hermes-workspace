...
function parseContext(content: string): Subject[] {
  ...
  for (const line of lines) {
    ...
    const deadline = line.match(/@deadline\((\d{4}-\d{2}-\d{2})\)|DEADLINE:\s*(\d{1,2}\/\d{2}\/\d{4})/i)
  ...
  return (
    <div className="flex min-h-0 flex-col gap-6 p-4 md:p-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold text-[var(--theme-text)]">University</h1>
          <p className="text-sm text-[var(--theme-muted)]">Obsidian brain synced from home PC</p>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="mb-3 flex items-start justify-between">
                  <div>
                    <h2 className="text-base font-semibold text-[var(--theme-text)]">{sub.name}</h2>
                    <span className="text-xs text-[var(--theme-muted)]">{sub.code}</span>
                  </div>
                  {sub.deadline && (
                  ...
                  <div className="mb-3 flex flex-col gap-1">
                    {sub.tasks.map((t, i) => (
                      <div key={i} className="flex items-center gap-2 text-sm">
                        <div className={`h-3.5 w-3.5 rounded border ${t.done ? 'bg-emerald-500 border-emerald-500' : 'border-[var(--theme-border)]'}`}>
                          {t.done && (
                        ...
                        <span className={t.done ? 'text-[var(--theme-muted)] line-through' : 'text-[var(--theme-text)]'}>
                          {t.text}
                        </span>
                      </div>
                    ))}
                ...
                {sub.notes.length > 0 && (
                  <div className="flex flex-col gap-1 border-t border-[var(--theme-border)] pt-2">
                    {sub.notes.map((n, i) => (
                      <div key={i} className="text-xs text-[var(--theme-muted)]">{n}</div>
                    ))}
                  </div>
                )}
              </div>
      ...
      <div className="flex flex-wrap gap-2">
        <a href="/uni/chat" className="rounded border border-[var(--theme-border)] bg-[var(--theme-card)] px-3 py-1.5 text-xs text-[var(--theme-text)] hover:border-[var(--theme-accent)]">
          → Chat with UniChat
        </a>
        <a href="/uni/calendar" className="rounded border border-[var(--theme-border)] bg-[var(--theme-card)] px-3 py-1.5 text-xs text-[var(--theme-text)] hover:border-[var(--theme-accent)]">
          → Calendar
        </a>
        <a href="/uni/moodle" className="rounded border border-[var(--theme-border)] bg-[var(--theme-card)] px-3 py-1.5 text-xs text-[var(--theme-text)] hover:border-[var(--theme-accent)]">
          → Moodle
        </a>
        <a href="/uni/obsidian" className="rounded border border-[var(--theme-border)] bg-[var(--theme-card)] px-3 py-1.5 text-xs text-[var(--theme-text)] hover:border-[var(--theme-accent)]">
          → Open Vault
        </a>
      </div>
    </div>
