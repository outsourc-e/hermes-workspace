...
const CLINITRACK_BASE = '/home/nick-weiland-oc381816/Projects/Praxentis/active/CliniTrack'
...
function ProjectsRoute() {
  ...
  return (
    <div className="flex min-h-0 flex-col gap-6 p-4 md:p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--theme-accent)] bg-opacity-20">
            <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current text-[var(--theme-accent)]">
              <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z"/>
            </svg>
          </div>
          <div>
            <h1 className="text-xl font-semibold text-[var(--theme-text)]">Projects</h1>
      ...
      {loading && (
        <div className="text-sm text-[var(--theme-muted)]">Loading worktrees…</div>
      )}
      ...
      {!loading && !error && (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {worktrees.map(wt => (
                <tr key={wt.path} className="border-b border-[var(--theme-border)]/50">
                  <td className="py-2 pr-4">
                    <div className="flex items-center gap-1.5">
                      <div className={`h-2 w-2 rounded-full ${statusColor(wt.status)}`} />
                      <span className="text-xs text-[var(--theme-muted)]">
                        {statusLabel(wt.status)}
                      </span>
                      {wt.ahead > 0 && (
                        <span className="text-[10px] text-emerald-500">↑{wt.ahead}</span>
                      )}
                      {wt.behind > 0 && (
                        <span className="text-[10px] text-amber-500">↓{wt.behind}</span>
                      )}
                    </div>
                  </td>
