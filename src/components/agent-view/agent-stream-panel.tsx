...
const ROLE_BADGE_CLASS: Record<string, string> = {
  ...
  system: 'bg-neutral-200 text-[var(--theme-text)]',
...
export function AgentStreamPanel({ sessionKey, agentName, agentColor, onClose }: AgentStreamPanelProps) {
  ...
  return (
    <>
      <button type="button" aria-label="Close live stream panel" className="fixed inset-0 z-40 bg-[color-mix(in_srgb,var(--theme-bg)_55%,black)]" onClick={onClose} />
      <aside className="fixed inset-x-0 bottom-0 z-50 h-[70vh] rounded-t-2xl border-t border-[var(--theme-border)] bg-[var(--theme-card)] shadow-2xl md:right-0 md:bottom-0 md:left-auto md:top-[var(--titlebar-h,0px)] md:h-auto md:w-[400px] md:rounded-none md:border-t-0 md:border-l">
        <div className="flex h-full min-h-0 flex-col">
          <div className="sticky top-0 z-10 border-b border-[var(--theme-border)] bg-[var(--theme-card)]/95 backdrop-blur">
            ...
            <div className="flex flex-wrap gap-2 px-4 pb-3">
              <span className="inline-flex items-center gap-1 rounded-lg bg-[var(--theme-card2)] px-2 py-1 text-xs"><span className="text-[var(--theme-muted)]">Tokens</span><span className="font-medium text-[var(--theme-text)]">{inputTokens.toLocaleString()} / {outputTokens.toLocaleString()}</span></span>
              <span className="inline-flex items-center gap-1 rounded-lg bg-[var(--theme-card2)] px-2 py-1 text-xs"><span className="text-[var(--theme-muted)]">Cost</span><span className="font-medium text-[var(--theme-text)]">{formatCost(totalCost)}</span></span>
              <span className="inline-flex items-center gap-1 rounded-lg bg-[var(--theme-card2)] px-2 py-1 text-xs"><span className="text-[var(--theme-muted)]">Runtime</span><span className="font-medium text-[var(--theme-text)]">{formatRuntime(runtimeStart)}</span></span>
            </div>
          </div>
          ...
          <div className="sticky bottom-0 border-t border-[var(--theme-border)] bg-[var(--theme-card)]/95 px-3 py-3 backdrop-blur">
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="relative">
                {menuOpen ? <div className="absolute bottom-full left-0 mb-2 w-full rounded-lg border border-[var(--theme-border)] bg-[var(--theme-card)] p-1 shadow-lg"><button type="button" onClick={() => void onPauseToggle()} disabled={pausePending} className="flex w-full rounded-md px-2 py-1.5 text-left text-xs text-[var(--theme-text)] hover:bg-[var(--theme-card2)] disabled:opacity-60">{pausePending ? 'Updating...' : isPaused ? 'Resume' : 'Pause'}</button><button type="button" onClick={() => void onKill()} disabled={killPending} className="flex w-full rounded-md px-2 py-1.5 text-left text-xs text-red-600 hover:bg-red-50 disabled:opacity-60 dark:text-red-300 dark:hover:bg-red-950/40">{killPending ? 'Terminating...' : 'Kill'}</button></div> : null}
