...
export function RouterChat({
  ...
  // Voice -> orchestrator: tap the mic, speak the mission, text lands in the
  // prompt box. Same transcription path as the main chat composer: server
  // /api/transcribe when a remote STT provider is configured, otherwise the
  // hook falls back to the browser's SpeechRecognition.
  const transcribeVoiceBlob = useCallback(async (blob: Blob) => {
    const form = new FormData()
    const extension = blob.type.includes('mp4') ? 'mp4' : 'webm'
    form.set('file', blob, `voice-mission.${extension}`)
    ...
    if (!response.ok || payload.ok === false || !payload.text) {
  ...
  const voiceInput = useVoiceInput({
    ...
    onResult: useCallback((text: string) => {
      setPrompt((cur) => (cur ? `${cur.trimEnd()} ${text}` : text))
    }, []),
  })
              ...
              <div className="flex items-center gap-2">
                ...
                {voiceInput.isSupported ? (
                  <button
                    ...
                    className={cn(
                      'inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-[11px] uppercase tracking-[0.18em]',
                      voiceInput.isListening
                        ? 'border-[var(--theme-danger-border)] bg-[var(--theme-danger-soft)] text-[var(--theme-text)]'
                        : 'border-[var(--theme-border)] bg-[var(--theme-card)] text-[var(--theme-muted)] hover:bg-[var(--theme-card2)] hover:text-[var(--theme-text)]',
                    )}
                  >
