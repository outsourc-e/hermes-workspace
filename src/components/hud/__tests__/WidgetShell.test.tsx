...
describe('WidgetShell', () => {
  ...
  it('shows skeleton when state=loading', () => {
    render(<WidgetShell state="loading" title="Agents"><div /></WidgetShell>);
  ...
  it('shows error badge when state=errored', () => {
    render(
      <WidgetShell state="errored" title="Agents" error={{ message: 'fetch failed' }}>
  ...
  it('returns null when state=disabled', () => {
    const { container } = render(
      <WidgetShell state="disabled" title="Agents"><div /></WidgetShell>
    );
