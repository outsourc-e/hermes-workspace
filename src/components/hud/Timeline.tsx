...
function TimelineImpl({ events, nowMin }: TimelineProps) {
  ...
  return (
    <div className="bg-[#161b22] rounded-lg p-3 relative h-24">
      <div className="text-[11px] text-[#8b949e] tracking-[0.15em] uppercase font-semibold">Today · 6am → 8pm</div>
      {events.map(ev => (
        <div
          key={ev.id}
      ...
      <div className="absolute top-7 bottom-2 w-px bg-white shadow-[0_0_6px_rgba(255,255,255,0.6)] z-10" style={{ left: pct(nowMin) }}>
        <span className="absolute -top-3 -left-3 text-[9px] text-white bg-[#0a0e14] px-1.5 py-0.5 rounded font-bold">NOW</span>
      </div>
    </div>
  );
}
