...
export function Tile({ state, label, value, sub, tone='info', fetchedAt, error }: TileProps) {
  return (
    <WidgetShell state={state} title={label} fetchedAt={fetchedAt} error={error} className="bg-[#161b22] border border-[#21262d] rounded-lg p-3 min-h-[68px] flex flex-col justify-between">
      <h4 className="text-[10px] text-[#58a6ff] uppercase tracking-[0.15em] font-semibold m-0">{label}</h4>
