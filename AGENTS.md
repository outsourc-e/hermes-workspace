...
> **Operating a cloud session on this workspace? Read
> [`SWARM-OPERATIONS-PLAYBOOK.md`](./SWARM-OPERATIONS-PLAYBOOK.md) FIRST.** It is
> the durable record of what breaks, recurring bugs to re-check every session,
> exact fix commands, and the build/verify/ship workflow. Update its change log
> when you land a structural fix.
